            ...
            <form action="{{ route('testimonials.update', ['testimonial' => $testimonial->id]) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('rating') is-invalid @enderror" id="rating" name="rating">
                        <option value="5_star" {{ old('rating', $testimonial->rating) ===  '5_star' ? 'selected' : '' }}>5 Star</option>
                        <option value="4_5_star" {{ old('rating', $testimonial->rating) ===  '4_5_star' ? 'selected' : '' }}>4.5 Star</option>
                    </select>
                    @error('rating')
                    <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
